...
if raster_layer is None or not isinstance(raster_layer, QgsRasterLayer):
    ...
    output_filename = f"{raster_name_without_ext}_vulnerabilidade.tif"
    ...
    # Expressão correta para a reclassificação
    expression = (
        f"(({raster_calc_name} >= 0.0000 AND {raster_calc_name} <= 5.0000) * 5) + "
        f"(({raster_calc_name} > 5.0000 AND {raster_calc_name} <= 10.0000) * 4) + "
        f"(({raster_calc_name} > 10.0000 AND {raster_calc_name} <= 15.0000) * 2) + "
        f"(({raster_calc_name} > 15.0000 AND {raster_calc_name} <= 25.0000) * 1) + "
        f"(({raster_calc_name} > 25.0000 AND {raster_calc_name} <= 30.0000) * 3) + "
        f"(({raster_calc_name} > 30.0000 AND {raster_calc_name} <= 35.0000) * 4) + "
        f"(({raster_calc_name} > 35.0000 AND {raster_calc_name} <= 50.0000) * 5)"
    )
    ...
    try:
        ...
        if result and 'OUTPUT' in result:
            output_layer = QgsRasterLayer(output_path, f"{raster_name_without_ext}_vulnerabilidade")
